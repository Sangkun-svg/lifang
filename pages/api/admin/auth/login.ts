...
import { isAdminUser, mapAdminSessionUser } from '@/lib/auth/admin';
import { clearAdminAuthCookies, setAdminAuthCookies, setLocalDevAdminAuthCookies } from '@/lib/auth/cookies';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponse<LoginData>>) {
  ...
  if (shouldBypassAuth) {
    setLocalDevAdminAuthCookies(res);
  ...
  try {
    ...
    if (error || !data.session || !data.user) {
      if (error) {
        console.warn('Admin Supabase login rejected', {
          code: error.code,
    ...
    if (!isAdminUser(data.user)) {
      console.warn('Admin login forbidden', {
      ...
      clearAdminAuthCookies(res);
      return sendFailure(res, 403, 'ADMIN_FORBIDDEN', '관리자 권한이 없습니다.');
    }

    setAdminAuthCookies(res, data.session);

    return sendSuccess(res, {
      user: mapAdminSessionUser(data.user),
      redirectTo: '/admin',
    });
  } catch (error) {
